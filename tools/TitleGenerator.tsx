...
const TitleGenerator: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await generateSimpleList(topic, 'titles');
      setTitles(result);
    } catch (err: any) {
  ...
  return (
    <ToolContainer
      ...
      description="Enter a topic or a brief description of your video, and Medusa will generate a list of catchy and engaging titles for you."
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <input
          ...
          placeholder="e.g., 'How to make the best chocolate chip cookies'"
      ...
      {titles && (
        <div className="mt-8">
            ...
            <OutputCard title="Generated Titles" content={titles} variant="list" />
        </div>
      )}
    </ToolContainer>
  );
